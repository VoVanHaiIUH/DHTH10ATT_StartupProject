...
namespace PresentationTier.CongNo
{
    public partial class frmTraNoKH : Form
    {
        ...
        {
            List<eCongNoKhachHang> l = CN.GetListNoCuaKhachHang(pMaKH);
            pRoot.Nodes.Clear();
            // tổng sô nợ
        ...
        private void LoadTinhTrangCongNo(TreeNode pRoot, string pTag)
        {
            ...
            List<eChiTietPhieuTraNoKhachHang> l = CN.GetChiTietTraNoKH(maHoaDon);
            dgvPhieuNo.DataSource = l;
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                //decimal.Parse(tbxSoTien.Text);
                if (null != CN.ThuNoKH(tbxMaHoaDon.Text, NhanVien.MaNhanVien, decimal.Parse(tbxSoTien.Text), ricGhiChu.Text))
                {
                    LoadTrv();
                ...
                MessageBox.Show("Thu nợ không thành công");
            }
