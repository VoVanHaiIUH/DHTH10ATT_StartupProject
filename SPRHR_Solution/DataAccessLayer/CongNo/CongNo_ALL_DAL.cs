...
namespace DataAccessLayer.CongNo
{
    public class CongNo_ALL_DAL
    {
        ...
        public List<eCongNoNCC> GetListNoCuaNhaCungCap(string pMaNhaCungCap)
        {
        ...
        public List<eCongNoKhachHang> GetListNoCuaKhachHang(string pMaKhachHang)
        {
        ...
        private string TaoMaChiTietPhieuTraNoNCC()
        {
            try
            {
                string maChiTietPhieuEnd = db.ChiTietPhieuTraNoNCCs.OrderByDescending(e => e.MaChiTietPhieuTraNoNCC).FirstOrDefault().MaChiTietPhieuTraNoNCC;
        ...
        private string TaoMaChiTietPhieuTraNoKH()
        {
            try
            {
                string maChiTietPhieuEnd = db.ChiTietPhieuTraNoKhachHangs.OrderByDescending(e => e.MaChiTietPhieuTraNoKH).FirstOrDefault().MaChiTietPhieuTraNoKH;
        ...
        private void KiemTraTraNoNCC(string pMaHoaDon, decimal pSoTienTra)
        {
            ...
            var querySoTienDaTra = db.ChiTietPhieuTraNoNCCs.Where(e => e.MaHoaDonNCC == pMaHoaDon);
            if (querySoTienDaTra.ToList().Count > 0)
                soTienDaTra = querySoTienDaTra.Sum(e => e.SoTien);
        ...
        private void KiemTraTraNoKH(string pMaHoaDon, decimal pSoTienTra)
        {
            ...
            if (db.CongNoKHs.Where(e => e.MaHoaDonBanSi == pMaHoaDon).FirstOrDefault() == null)
                throw new Exception("Không có công nợ cho hóa đơn này");
            decimal soTienDaTra = 0;
            var querySoTienDaTra = db.ChiTietPhieuTraNoKhachHangs.Where(e => e.MaHoaDonBanSi == pMaHoaDon);
            if (querySoTienDaTra.ToList().Count > 0)
                soTienDaTra = querySoTienDaTra.Sum(e => e.SoTien);
        ...
        public IEnumerable<object> GetListNoQuaHan()
        {

            var query = db.CongNoKHs.Where(e => e.NgayDaoHan.CompareTo(DateTime.Now) == -1 &&
            e.ChiTietPhieuTraNoKhachHangs.Sum(d => d.SoTien) < e.SoTienNo).Select(z => new
            {
                z.HoaDonBanSi.KhachHang.TenKH,
                z.MaHoaDonBanSi,
                z.SoTienNo,
                z.NgayDaoHan
            });
        ...
        public IEnumerable<object> GetListNoDaTra()
        {
            var query = db.ChiTietPhieuTraNoKhachHangs;
            if (query.ToList().Count == 0)
                return null;
            return query;
        }
        public IEnumerable<object> GetListNoToiHan()
        {
            // được xem là sắp tới hạn khi còn 10 ngày nữa là tới hạn trả
            var query = db.CongNoKHs.Where(e => DateTime.Now.CompareTo(e.NgayDaoHan) == -1 &&
            e.ChiTietPhieuTraNoKhachHangs.Sum(d => d.SoTien) < e.SoTienNo && (DateTime.Now - e.NgayDaoHan).TotalDays == 10).Select(z => new
            {
                z.HoaDonBanSi.KhachHang.TenKH,
                z.MaHoaDonBanSi,
                z.SoTienNo,
        ...
        public List<eCongNoNCC> GetListNoCuaNhaCungCap(DateTime pNgayBatDau, DateTime pNgayKetThuc)
        {
            List<eCongNoNCC> l = new List<eCongNoNCC>();
            foreach (var cn in db.CongNoNCCs.Where(e => e.HoaDonNhaCungCap.NgayLap >= pNgayBatDau && e.HoaDonNhaCungCap.NgayLap <= pNgayKetThuc))
            {
        ...
        public List<eCongNoKhachHang> GetListNoCuaKhachHang(DateTime pNgayBatDau, DateTime pNgayKetThuc)
        {
            List<eCongNoKhachHang> l = new List<eCongNoKhachHang>();
            foreach (var cn in db.CongNoKHs.Where(e => e.HoaDonBanSi.NgayLap >= pNgayBatDau && e.HoaDonBanSi.NgayLap <= pNgayKetThuc))
            {
